...
range returns [ArrayList<Expression> rangeRet]:
    ...
    int1 = INT_VALUE {var tmp1 = new IntValue(Integer.parseInt($int1.text)); tmp1.setLine($int1.line); $rangeRet.add(tmp1);})
    COMMA
    ( id2 = IDENTIFIER {$rangeRet.add(Identifier.createId($id2.text, $id2.line));} |
    int2 = INT_VALUE {var tmp2 = new IntValue(Integer.parseInt($int2.text)); tmp2.setLine($int2.line); $rangeRet.add(tmp2);})
    RPAR
)
;
