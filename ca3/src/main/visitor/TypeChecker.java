...
import static main.ast.nodes.expression.operator.BinaryOperator.EQUAL;
...
public class TypeChecker extends Visitor<Type>{
    ...
    public Type visit(ForStatement forStatement) {
        ...
        if (forStatement.getConditions().size() == 1){
            if (forStatement.getConditions().get(0) instanceof Identifier){
                if (!(forStatement.getConditions().get(0).accept(this) instanceof ArrayType)){
                    typeErrors.add(new IsNotIterable(forStatement.getConditions().get(0).getLine()));
                }
            }
        }
    ...
    public Type visit(CallExpression callExpression) {
        ...
        if (Objects.equals(handlerName, "print")) {
            // Error 4
            Type argType = callExpression.getExpressions().accept(this);
            Type strType = new StringType();
            if (!argType.hasSameType(strType)){
            ...
            Type addType = callExpression.getExpressions().accept(this);
            Type listType = currentCaller.accept(this);
            if (!listType.hasSameType(addType)){
    ...
    private ActorDec getActor(Expression currentCaller) {
        ...
        var actorName = currentCaller.accept(this);
        try {
            return ((ActorDecItem) currentSymbolTable.getItem(ActorDecItem.START_KEY + actorName)).getActorDec();
        } catch (ItemNotFound e) {
        }
        // Error 8
        return null;
...
//        }
    ...
    public Type visit(UnaryExpression unaryExpression) {
        ...
        if (unaryOp == UnaryOperator.NOT){
            if(!(new BooleanType()).hasSameType(unaryType)){
                typeErrors.add(new UnsupportedOperandType(unaryExpression.getLine(), unaryOp.toString()));
                return returnType;
            }
            return new BooleanType();
        }

        var intOP = new ArrayList<UnaryOperator>(Arrays.asList(UnaryOperator.POST_DEC, UnaryOperator.POST_INC, UnaryOperator.POST_INC, UnaryOperator.POST_INC, UnaryOperator.MINUS));
        if (intOP.contains(unaryOp)){
            if(!(new IntType()).hasSameType(unaryType)){
                typeErrors.add(new UnsupportedOperandType(unaryExpression.getLine(), unaryOp.toString()));
                return returnType;
            }
            return new IntType();
    ...
    public Type visit(InitStatement assignmentStatement) {
        ...
        if (assignmentStatement.getAssigned() instanceof ExpressionList) {
            ExpressionList expList = (ExpressionList) assignmentStatement.getAssigned();
            Type expType = expList.accept(this);
            if (!(assineeType instanceof ArrayType)){
                typeErrors.add(new NonSameOperands(assignmentStatement.getLine(), EQUAL));
                return new NoType();
            }
            Type innerType = ((ArrayType) assineeType).getType();
            if (!expType.hasSameType(innerType)){
                typeErrors.add(new NonSameOperands(assignmentStatement.getLine(), EQUAL));
                return new NoType();
            }
        } else {
            if (!assignmentStatement.getAssigned().accept(this).hasSameType(assineeType)){
                typeErrors.add(new NonSameOperands(assignmentStatement.getLine(), EQUAL));
                return new NoType();
            }
        }
    ...
    public Type visit(ExpressionList expressionList) {
        ...
        // Error 1
        if (expressionList.getExpressionList().size() == 0){
            return new NoType();
        }

        Type type = expressionList.getExpressionList().get(0).accept(this);
        for (Expression exp : expressionList.getExpressionList()){
            if (!type.hasSameType(exp.accept(this))){
...
//                typeErrors.add(new UnsupportedOperandType(binaryExpression.getLine(), binOp.name()));
            ...
            //
            Type indexType = binaryExpression.getRightOperand().accept(this);
            Type intType = new IntType();
            if (!indexType.hasSameType(intType)){
...
//            exprTypes.add(observers.accept(this));
        ...
        // Error 8
        for (Type type : exprTypes){
            if (!type.hasSameType(new SetType(new IdType()))){
